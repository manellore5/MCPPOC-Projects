...
const mcpServerPort = await getPort({
  ...
  exclude: [process.env.PORT, inspectorServerPort, inspectorClientPort].filter(Boolean).map(Number),
});
...
function startDevServer() {
  ...
  console.log(styleText("yellow", `🔍 Starting mcp dev server on port ${mcpServerPort}...`));

  devServerProcess = execa("npm", ["--silent", "--prefix", process.cwd(), "run", "dev:server"], {
    stdio: ["ignore", "pipe", "pipe"],
    env: { ...process.env, PORT: mcpServerPort },
  });

  process.on("error", (err) => {
    console.error(styleText("red", "❌ Dev server failed to start:"), err.message);
  });
...
function startInspector() {
  ...
  inspectorProcess.on("error", (err) => {
    console.error(styleText("red", "❌ Inspector failed to start:"), err.message);
  });
...
async function startServers() {
  ...
  await Promise.all([
    waitForServerReady({
      ...
      name: "[DEV-SERVER]",
    }).then(() => waitForResourceReady(`http://localhost:${mcpServerPort}/healthcheck`)),
    waitForServerReady({
      process: inspector,
  ...
  console.log(
    styleText("green", servers.length > 1 ? "✅ All servers ready!" : "✅ Server ready!"),
  );
}
  ...
  const server = createServer((req, res) => {
    if (req.url === "/" || req.url.startsWith("/?")) {
      ...
      if (transport === "stdio") {
        ...
        url.searchParams.set("serverUrl", `http://localhost:${mcpServerPort}/mcp`);
      }

      url.searchParams.set("MCP_PROXY_AUTH_TOKEN", sessionToken);
      url.searchParams.set("MCP_PROXY_FULL_ADDRESS", `http://localhost:${inspectorServerPort}`);
...
function startProxyServer(server) {
  server.listen(proxyPort, () => {
    ...
    console.log(styleText("cyan", `🐨 Proxy server running: `) + styleText("green", proxyUrl));
    console.log(
    ...
    if (transport === "streamable-http") {
      console.log(
        styleText("gray", `- MCP server port: `) + styleText("yellow", mcpServerPort.toString()),
      );
    }
  });
}
