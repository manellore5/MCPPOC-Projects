...
export function meta({}: Route.MetaArgs) {
  return [{ title: "Epic Me" }, { name: "description", content: "The Epic Me Journaling App" }];
}
    ...
    users.map(async (user) => {
      ...
      const fullEntries = await Promise.all(
        entries.map(async (entry) => {
          ...
          return {
            ...
            formattedDate: new Date(fullEntry.createdAt * 1000).toLocaleDateString("en-US", {
              year: "numeric",
              month: "short",
              day: "numeric",
              hour: "2-digit",
      ...
      const fullTags = await Promise.all(
        tags.map(async (tag) => {
          ...
          return {
            ...fullTag,
            formattedDate: new Date(fullTag.createdAt * 1000).toLocaleDateString("en-US", {
              year: "numeric",
              month: "short",
              day: "numeric",
              hour: "2-digit",
      ...
      const validTags = fullTags.filter((tag): tag is NonNullable<typeof tag> => tag !== null);
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          <div className="mt-1 flex items-center gap-4 text-sm text-gray-600 dark:text-gray-300">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400">ID: {entry.id}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border border-gray-200 bg-white px-3 py-2 dark:border-gray-600 dark:bg-gray-600">
      <div className="flex items-center gap-3">
        <span className="text-xs text-gray-500 dark:text-gray-400">ID: {tag.id}</span>
        <span className="inline-flex items-center rounded-full bg-blue-100 px-2 py-1 text-xs font-medium text-blue-800 dark:bg-blue-900 dark:text-blue-200">
          {tag.name}
        </span>
        {tag.description && (
          <span className="text-xs text-gray-600 dark:text-gray-300">{tag.description}</span>
        )}
      </div>
  ...
  return (
    <details className="group rounded-xl bg-gray-50 dark:bg-gray-700 [&>summary]:list-none">
      ...
      <div className="border-t border-gray-200 p-6 dark:border-gray-600">
        <div className="space-y-3">
          <div>
            <h4 className="text-sm font-medium text-gray-600 dark:text-gray-400">Value</h4>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="font-medium text-gray-600 dark:text-gray-400">Expiration:</span>
              <span className="ml-2 text-gray-900 dark:text-white">
                {kvEntry.expiration
                  ? new Date(kvEntry.expiration * 1000).toLocaleDateString("en-US", {
                      year: "numeric",
                      month: "short",
                      day: "numeric",
                      hour: "2-digit",
...
function DatabaseResetSection() {
  return (
    <section aria-labelledby="db-reset-heading">
      <h2 id="db-reset-heading" className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">
        Database Reset
      </h2>
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-lg dark:border-gray-700 dark:bg-gray-800 dark:shadow-gray-900/50">
        <p className="mb-6 text-gray-600 dark:text-gray-300">
          This allows you to completely reset the database and KV store. This will delete all data
          and rerun migrations to recreate the schema with fresh data.
        </p>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
                <p>This action will permanently delete all data in the database and KV store.</p>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-8 dark:from-gray-900 dark:to-gray-800">
      <div className="mx-auto max-w-6xl">
        <header>
          <h1 className="mb-8 text-4xl font-bold text-gray-900 dark:text-white">Epic Me App</h1>
        </header>
