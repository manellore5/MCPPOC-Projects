...
async function getTokenInfo(request: Request, env: Env): Promise<Token | undefined> {
  const token = request.headers.get("authorization")?.slice("Bearer ".length);
  if (!token) return undefined;
  return resolveTokenInfo(token, env);
}

async function resolveTokenInfo(token: string, env: Env): Promise<Token | undefined> {
