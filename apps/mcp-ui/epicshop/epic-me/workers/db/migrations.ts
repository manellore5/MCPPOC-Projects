  ...
  },
  {
    ...
    up: async (db: D1Database) => {
      ...
      try {
        ...
        const createUserWithData = async (
          ...
          for (const mapping of userData.entryTagMappings) {
            ...
            for (const tagName of mapping.tagNames) {
              const tagIndex = userData.tags.findIndex((t) => t.name === tagName);
              if (tagIndex !== -1) {
          ...
          console.log(`Applied ${totalTagApplications} tag relationships for ${email}`);
        };
        ...
        console.log("Successfully created all users with their tags and entries");
      } catch (error) {
  ...
  try {
    ...
    for (const migration of migrations) {
      if (migration.version > currentVersion) {
        ...
        await db
          .prepare(sql`INSERT INTO schema_versions (version, name) VALUES (?, ?);`)
