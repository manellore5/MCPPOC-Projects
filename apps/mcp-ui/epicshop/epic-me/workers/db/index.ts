...
export class DB {
  ...
  async createEntry(userId: number, entry: z.input<typeof newEntrySchema>) {
    ...
    const createdEntry = await this.getEntry(userId, insertResult.meta.last_row_id);
    if (!createdEntry) {
  ...
  async getEntries(userId: number, tagIds?: number[], from?: string, to?: string) {
    const queryParts = [
  ...
  async updateEntry(userId: number, id: number, entry: Partial<z.input<typeof newEntrySchema>>) {
  ...
  async createTag(userId: number, tag: z.input<typeof newTagSchema>) {
    ...
    const insertResult = await ps.bind(validatedTag.name, validatedTag.description, userId).run();
  ...
  async updateTag(userId: number, id: number, tag: Partial<z.input<typeof newTagSchema>>) {
  ...
  async addTagToEntry(userId: number, entryTag: NewEntryTag) {
    ...
    const created = await this.getEntryTag(userId, insertResult.meta.last_row_id);
    if (!created) {
  ...
  async getEntryTags(userId: number, entryId: number) {
    ...
    return z.array(tagSchema).parse(results.results.map((result) => snakeToCamel(result)));
  }
}
