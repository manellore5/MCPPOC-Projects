...
import { getApps, getAppDisplayName } from "@epic-web/workshop-utils/apps.server";
...
async function main() {
  ...
  if (process.argv[2]) {
    ...
    selectedApps = allAppsWithTests.filter((app) => {
      ...
      return patterns.some((pattern) => {
        let [patternExercise = "*", patternStep = "*", patternType = "*"] = pattern.split(".");
        ...
        return (
          (patternExercise === "*" || exerciseNumber === Number(patternExercise)) &&
    ...
    const displayNameMap = new Map(
      allAppsWithTests.map((app) => [getAppDisplayName(app, allAppsWithTests), app]),
    );
    ...
    selectedApps = response.appDisplayNames.includes("All")
      ? allAppsWithTests
      : response.appDisplayNames.map((appDisplayName) => displayNameMap.get(appDisplayName));
    ...
    const appPattern =
      selectedApps.length === allAppsWithTests.length
        ...
        : selectedApps
            ...
            .join(",");
    const additionalArgsString = additionalArgs.length > 0 ? ` -- ${additionalArgs.join(" ")}` : "";
  ...
  if (selectedApps.length === 1) {
    ...
    try {
      ...
      console.log(`✅ Finished tests for ${app.relativePath} (${duration.toFixed(2)}s)`);
    } catch {
      const duration = (performance.now() - startTime) / 1000;
      console.error(`❌ Tests failed for ${app.relativePath} (${duration.toFixed(2)}s)`);
      process.exit(1);
    }
  } else {
    ...
    const tasks = selectedApps.map((app) =>
      limit(async () => {
        ...
        const startTime = performance.now();
        try {
          const subprocess = execa("npm", ["run", "test", "--silent", "--", ...additionalArgs], {
