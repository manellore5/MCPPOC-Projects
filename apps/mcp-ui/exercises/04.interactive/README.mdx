...
<EpicVideo url="https://www.epicai.pro/workshops/mcp-ui-aubv9/intro-to-interactive-gcc2n" />
...
await sendMcpMessage("prompt", {
  prompt: "Create a recipe for a fusion dish combining Japanese and Mexican cuisine",
});
```

<callout-info>
  The MCP UI communication protocol uses `postMessage` to enable secure, bidirectional communication
  between iframe-based UI components and their host application. This allows iframes to trigger
  actions in the host while maintaining security boundaries.
</callout-info>
...
<callout-info>
  `intent` and `notify` are not used in this exercise as they are not relevant for general use AI
  agent apps and typically require specific server-client integration.
</callout-info>
