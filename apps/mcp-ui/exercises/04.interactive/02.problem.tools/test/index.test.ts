...
test("journal viewer sends tool message", async () => {
  ...
  const { resource } = z.object({ resource: z.object({}).passthrough() }).parse(result.content[0]);
  ...
  const viewDetailsButton = iframe.getByRole("button", { name: "View Details" }).first();
  ...
  expect(messageContent, "🚨 the tool message is not the correct format").toEqual({
    type: "tool",
