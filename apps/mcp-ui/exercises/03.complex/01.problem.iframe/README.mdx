...
<callout-warning>
  We need to add a custom `x-origin` header to the MCP request so our tool handler knows where to
  set the full iframe URL. This header is added in the worker's fetch handler before forwarding the
