...
export async function initializeTools(agent: EpicMeMCP) {
  ...
  agent.server.registerTool(
    ...
    async ({ id }) => {
      ...
      return {
        structuredContent,
        content: [createEntryResourceLink(entry), createText(structuredContent)],
      };
    },
  );
  ...
  agent.server.registerTool(
    ...
    async ({ id, ...updates }) => {
      ...
      return {
        structuredContent,
        content: [
          createText(`Entry "${updatedEntry.title}" (ID: ${id}) updated successfully`),
    ...
    async ({ id }) => {
      ...
      if (!confirmed) {
        ...
        return {
          structuredContent,
          content: [
            createText(`Deleting entry "${existingEntry.title}" (ID: ${id}) rejected by the user.`),
            createText(structuredContent),
          ],
        };
      }
      ...
      return {
        structuredContent,
        content: [
          createText(`Entry "${existingEntry.title}" (ID: ${id}) deleted successfully`),
  ...
  agent.server.registerTool(
    ...
    async (tag) => {
      ...
      return {
        structuredContent,
        content: [
          createText(`Tag "${createdTag.name}" created successfully with ID "${createdTag.id}"`),
  ...
  agent.server.registerTool(
    ...
    async ({ id, ...updates }) => {
      ...
      return {
        structuredContent,
        content: [
          createText(`Tag "${updatedTag.name}" (ID: ${id}) updated successfully`),
    ...
    async ({ id }) => {
      ...
      if (!confirmed) {
        ...
        return {
          structuredContent,
          content: [
            createText(`Deleting tag "${existingTag.name}" (ID: ${id}) rejected by the user.`),
      ...
      return {
        structuredContent,
        content: [
          createText(`Tag "${existingTag.name}" (ID: ${id}) deleted successfully`),
          createTagResourceLink(existingTag),
          createText(structuredContent),
        ],
      };
  ...
  agent.server.registerTool(
    ...
    async ({ id }) => {
      const iframeUrl = new URL(`/ui/entry-viewer/${id}`, agent.requireBaseUrl());
...
function createEntryResourceLink(entry: { id: number; title: string }): ResourceLinkContent {
  return {
    type: "resource_link",
...
function createTagResourceLink(tag: { id: number; name: string }): ResourceLinkContent {
  return {
    type: "resource_link",
