...
<EpicVideo url="https://www.epicai.pro/workshops/mcp-ui-aubv9/intro-to-advanced-u3kkh" />
...
sequenceDiagram
    ...
    Note over User,MCP: Tool Results Flow
    User->>Iframe: Interact with UI (e.g., delete entry)
    Iframe->>App: Send tool message via postMessage
    App->>MCP: Call tool with structured output schema
    MCP->>MCP: Process request
    MCP->>App: Return structured result (success/failure)
    App->>Iframe: Send result via postMessage
    Iframe->>Iframe: Validate with Zod schema
    Iframe->>User: Update UI based on result

    Note over User,MCP: Render Data Flow
    User->>App: Request UI resource
    App->>MCP: Get UI resource with initial data
    MCP->>App: Return resource with uiMetadata containing render data
    App->>Iframe: Create iframe with resource
    Iframe->>App: Emit ui-lifecycle-iframe-ready event
    App->>Iframe: Send ui-lifecycle-iframe-render-data event
    Iframe->>Iframe: Process initial data
    Iframe->>User: Display content with pre-loaded data
```
...
<callout-info>
  With structured content, MCP clients can build sophisticated workflows where tools can trigger
  automatic actions based on their results, rather than requiring manual user intervention for every
  step.
</callout-info>
