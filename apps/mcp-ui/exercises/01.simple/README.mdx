...
<callout-info>
  The `createUIResource` function is the foundation of MCP UI. It takes your HTML content and
  packages it into a format that MCP clients can understand and render. The `uri` parameter creates
  a unique identifier for your UI resource, while the `content` specifies what type of UI you're
  providing.
</callout-info>
...
<callout-success>
  ...
  Raw HTML content is delivered as `text/html` MIME type and can include any valid HTML markup,
  making it ideal for simple UI components and static content display.
</callout-success>
