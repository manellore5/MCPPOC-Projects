...
<callout-info>
  The `createUIResource` function packages your HTML content into a format that MCP clients can
  understand and render. The `uri` parameter creates a unique identifier for your UI resource, while
  the `content` specifies what type of UI you're providing.
</callout-info>

When there's no tag found, we should also provide a clear visual indication rather than just text:

<callout-info>
  The styling in these examples uses inline CSS for simplicity, but you can use any valid HTML
  markup. Consider using CSS classes or even external stylesheets for more complex designs.
</callout-info>
