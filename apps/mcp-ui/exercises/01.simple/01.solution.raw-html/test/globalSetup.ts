...
export default async function setup(project: TestProject) {
  const mcpServerPort = process.env.PORT ? Number(process.env.PORT) : await getPort();
  ...
  async function waitForServerReady({
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      function searchForMatch(channel: OutputBuffer[number]["channel"], data: Buffer) {
  ...
  async function startMcpServerIfNecessary() {
    const isMcpRunning = await fetch(`http://localhost:${mcpServerPort}/healthcheck`).catch(() => ({
      ok: false,
    }));
    if (isMcpRunning.ok) {
      return;
      ...
      await Promise.all([
        appServerProcess
          ? waitForServerReady({
            ...
            }).then(() => waitForResourceReady("http://localhost:7787/healthcheck"))
          : Promise.resolve(),
        waitForServerReady({
